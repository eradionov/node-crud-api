...
export async function getParametersBag(route: RouteDefinition, req: http.IncomingMessage): Promise<Map<string, any>> {
    const params = getQueryParams(req);

    if ([HTTP_PUT, HTTP_POST].includes(req.method!)) {
        params.set('body', <{username?: string, age?: number, hobbies?: string[]}> await getBody(req));
    }

    Object.entries(
        getMappedPathParams(req.url!, route.path)
    )
        .forEach(([param, value]) => {
...
export function getQueryParams(req: http.IncomingMessage): Map<string, any> {
    ...
    const queryData = new URL(req.url!, `${protocol}://${req.headers.host}`).searchParams;
